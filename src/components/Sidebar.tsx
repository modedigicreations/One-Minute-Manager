...
export default function Sidebar({ profile }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* ============================================================ */}
      <header className="lg:hidden h-14 bg-[#0b1329]/95 backdrop-blur-md border-b border-slate-800/80 flex items-center justify-between px-4 text-white sticky top-0 z-40">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center text-slate-950 font-extrabold shadow-sm shadow-emerald-500/20">
            <Award size={16} />
          </div>
          <div className="flex items-center gap-1.5">
            <span className="font-extrabold text-sm tracking-tight text-white">One-Minute</span>
            <span className="text-[9px] font-bold font-mono px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/25 uppercase">
              Live
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="flex items-center gap-2 p-1.5 pl-2 rounded-xl bg-slate-900/90 border border-slate-800 text-slate-300 hover:text-white transition cursor-pointer"
      ...
      {isOpen && (
        <div
          ...
          className="lg:hidden fixed inset-0 bg-black/75 backdrop-blur-xs z-50 transition-opacity animate-in fade-in duration-200"
        />
      )}

      <div
        className={`lg:hidden fixed inset-y-0 left-0 w-[85vw] max-w-xs bg-[#0b1329] z-50 transform transition-transform duration-300 ease-in-out shadow-2xl flex flex-col ${
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div className="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-slate-800 to-slate-700 border border-slate-600/50 flex items-center justify-center font-extrabold text-emerald-400 text-sm">
              {userInitial}
            </div>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm font-semibold transition ${
                    isActive
                      ? 'bg-gradient-to-r from-emerald-500/20 via-emerald-500/10 to-transparent text-emerald-400 border-l-2 border-emerald-400'
          ...
          {/* Motivation Quote */}
          <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800/80 text-xs text-slate-400">
            <div className="flex items-center gap-1.5 text-emerald-400 font-bold mb-1">
              <Sparkles size={13} />
              <span>The Blanchard Axiom</span>
            </div>
            <p className="text-[11px] leading-relaxed text-slate-400 italic">
              &ldquo;Help people reach their full potential: Catch them doing something right.&rdquo;
            </p>
          </div>
        </div>
        ...
        <div className="p-5 border-t border-slate-800/80">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold text-rose-400 bg-rose-950/20 hover:bg-rose-950/40 border border-rose-900/30 transition cursor-pointer"
          >
            <LogOut size={15} />
            <span>Sign Out</span>
          </button>
      ...
      {/* ============================================================ */}
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-[#0b1329]/95 backdrop-blur-xl border-t border-slate-800 pb-safe">
